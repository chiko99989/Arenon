...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::are)
    {
    }
                                              ...
                                              filter(0)
{
    ...
    if (fLiteMode) {
        ...
        if (fMasterNode) {
            ...
            if (!fEnablearesend) {
                ui->toggleObfuscation->setText(tr("Start aresend"));
            } else {
                ui->toggleObfuscation->setText(tr("Stop aresend"));
            }
...
void SmartContractPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("are")
    updateDisplayUnit();
}
...
void SmartContractPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeareAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeareAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeareAmount = strAnonymizeareAmount.remove(strAnonymizeareAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeareAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeareAmount * COIN) nMaxToAnonymize = nAnonymizeareAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeareAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeareAmount));
        strAnonymizeareAmount = strAnonymizeareAmount.remove(strAnonymizeareAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeareAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
