...
areState::areState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = areState::openDB(_path + "/areDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

areState::areState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute areState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, areTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createareAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            are::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> areState::vins() const // temp
{
...
void areState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* areState::vin(dev::Address const& _a) const
{
    return const_cast<areState*>(this)->vin(_a);
}

Vin* areState::vin(dev::Address const& _addr)
{
...
// void areState::commit(CommitBehaviour _commitBehaviour)
// {
...
void areState::kill(dev::Address _addr)
{
...
void areState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address areState::createareAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void areState::deleteAccounts(std::set<dev::Address>& addrs){
...
void areState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void areState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
