...
#include "addeditarenode.h"
#include "arenodeconfigdialog.h"
...
static void NotifyareNodeUpdated(MasternodeManager *page, CareNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateareNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyareNodeChanged.connect(boost::bind(&NotifyareNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyareNodeChanged.disconnect(boost::bind(&NotifyareNodeUpdated, this, _1));
}
...
void MasternodeManager::updateareNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_adrenaline);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_adrenaline);
        BOOST_FOREACH(PAIRTYPE(std::string, CareNodeConfig) adrenaline, pwalletMain->mapMyareNodes)
        {
            updateareNode(QString::fromStdString(adrenaline.second.sAlias), QString::fromStdString(adrenaline.second.sAddress), QString::fromStdString(adrenaline.second.sMasternodePrivKey), QString::fromStdString(adrenaline.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditareNode* aenode = new AddEditareNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CareNodeConfig c = pwalletMain->mapMyareNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    areNodeConfigDialog* d = new areNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyareNodes.erase(sAddress);
        walletdb.EraseareNodeConfig(c.sAddress);
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    BOOST_FOREACH(PAIRTYPE(std::string, CareNodeConfig) adrenaline, pwalletMain->mapMyareNodes)
    {
        CareNodeConfig c = adrenaline.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
