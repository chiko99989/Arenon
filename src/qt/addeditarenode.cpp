...
AddEditareNode::AddEditareNode(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AddEditareNode)
{
    ui->setupUi(this);

}

AddEditareNode::~AddEditareNode()
{
    delete ui;
}


void AddEditareNode::on_okButton_clicked()
{
	...
	bool bForceNew = false;
        ...
        pwalletMain->mapMyareNodes.insert(make_pair(c.sAddress, c));
	walletdb.WriteareNodeConfig(c.sAddress, c);
        uiInterface.NotifyareNodeChanged(c);

        accept();
    }
}

void AddEditareNode::on_cancelButton_clicked()
{
    reject();
}
