...
bool areDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule areDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t areDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t areDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t areDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t areDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool areDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void areDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void areDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void areDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void areDGP::createParamsInstance(){
...
dev::Address areDGP::getAddressForBlock(unsigned int blockHeight){
...
void areDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void areDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void areDGP::parseStorageOneUint64(uint64_t& value){
...
void areDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule areDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
