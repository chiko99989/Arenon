...
// Implemented are parallel miner | Auto deploy
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("areMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("areMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("areMiner started (%s)\n", (fProofOfStake ? "PoS" : "PoW"));
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("are-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running areMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
